...
public class LoanController {
	...
	@GetMapping("/loans/{id}")
	@ResponseBody
	public ResponseEntity<?> getLoanCardDetails(@Validated @PathVariable String id) {
		Optional<LoanCard> loanCard = loanService.findById(id);
		if (loanCard.isEmpty()) {
			return new ResponseEntity<>("Loan Card not found.", HttpStatus.NOT_FOUND);
		}
		return new ResponseEntity<>(loanCard.get(), HttpStatus.OK);
	}
}
